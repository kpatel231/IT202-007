...
if (isset($_POST["item_id"])) {
    ...
    $db->beginTransaction();
    //deduct item
    $stmt = $db->prepare("UPDATE Cart set quantity = quantity - 1 WHERE item_id = :id");
    $usedItem = false;
    try {
    ...
    if ($usedItem) {
        $item = [];
        $stmt = $db->prepare("SELECT uses,quantity, name from Products i JOIN Cart inv on i.id = inv.item_id WHERE i.id = :id");
        try {
            $stmt->execute([":id" => $item_id]);
        ...
        if ($item) {
            $stmt = $db->prepare("INSERT INTO ActiveEffects (item_id, user_id, uses) VALUES (:iid, :uid, :u) ON DUPLICATE KEY UPDATE uses = uses + :u");
            try {
